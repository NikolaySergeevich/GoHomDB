...
.PHONY: docker
docker-run:
	docker run --name manager-pg -e POSTGRES_DB=users -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres -d -p "5434:5432" postgres
...
.PHONY: migrate-down
migrate-down:
	 migrate -source "file://./migrations" -database "postgres://localhost:5434/users?sslmode=disable&user=postgres&password=postgres" down
